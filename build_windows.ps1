...
uv pip install --python $venvPython --index-url $MIRROR pyinstaller PyQt6 2>&1 | Out-Host
...
if (Test-Path $distApp) {
    ...
    Get-ChildItem -Force $distApp | ForEach-Object {
        Move-Item -Force -LiteralPath $_.FullName -Destination (Join-Path $distRoot $_.Name)
    }
